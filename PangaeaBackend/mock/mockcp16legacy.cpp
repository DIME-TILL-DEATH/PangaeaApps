...
bool MockCP16Legacy::loadPresetData(quint8 prBank, quint8 prPreset, preset_data_cplegacy_t *presetData)
{
    ...
    if(presetFile.open(QIODevice::ReadOnly))
    {
        ...
        memcpy(presetData, readedData.data(), sizeof(preset_data_cplegacy_t));
        presetFile.close();
        return true;
    }
    else return false;
...
bool MockCP16Legacy::savePresetData(quint8 prBank, quint8 prPreset, const preset_data_cplegacy_t *presetData)
{
    QString dirPath = m_basePath + "/bank_" + QString().setNum(prBank) + "/preset_" + QString().setNum(prPreset);
    QFile presetFile(dirPath + "/preset.pan");
    ...
    {
        char rawData[sizeof(preset_data_cplegacy_t)];
        memcpy(rawData, &currentPresetData, sizeof(preset_data_cplegacy_t));
...
void MockCP16Legacy::getStateCommHandler(const QString &command, const QByteArray& arguments, const QByteArray &data)
{
    if(data.size() == 0)
    {
        quint8 rawData[sizeof(preset_data_cplegacy_t)];
        memcpy(rawData, &currentPresetData, sizeof(preset_data_cplegacy_t));

        QByteArray baData = QString("gs\r").toUtf8();

        for(int i=0; i < sizeof(preset_data_cplegacy_t);  i++)
        {
    ...
    else
    {
        ...
        if(dataList.size()>0)
        {
            ...
            memcpy(&currentPresetData, rawArr, sizeof(preset_data_cplegacy_t));
        }
