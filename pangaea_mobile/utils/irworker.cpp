...
stWavHeader IRWorker::getFormatWav(QString filePath)
{
    ...
    //TODO: должна вылетать ошибка "не могу открыть файл", а не "Неверный формат"
    if(!file.open(QIODevice::ReadOnly))
    {
        qWarning() << __FUNCTION__ << "Can't open file: " << filePath;
    }

    QByteArray baWav;
    baWav = file.read(44+4096);
    file.close();
...
QByteArray IRWorker::decode(QByteArray *data, quint8 lenSample, quint8 chanalCount)
{
    ...
    {
        ret.append(IRWorker::decodeSample(data->mid(i,lenSample), lenSample));
    }
...
const QByteArray& IRWorker::formFileData()
{
    ...
    //reinterpret_cast<const char*>(&chunkSize), sizeof(quint32)
    m_fileData.append(IRWorker::intToBa(data.length()));                      //subchunk2Size

    m_fileData.append(data);
    m_fileData.replace(4, 4, IRWorker::intToBa(m_fileData.length()-8));
...
QByteArray IRWorker::flatIr()
{
    ...
    baflatIr.append(0xFF); // QByteArray::fromRawData("\xFF\xFF\x7F", 3);
    baflatIr.append(0x7F);
    baflatIr.append(1024, 0x00);//983*3*2, 0x00);
