...
quint8 Preset::getPresetFlatIndex()
{
    return (m_bankNumber<<4) + m_presetNumber;
}

quint8 Preset::getPresetFlatNumber()
{
    quint16 maxBankPreset;

    if(m_ownerDevice == nullptr)
    {
        qWarning() << __FUNCTION__ << "Owner device doesn't set. Usimg default maxBank=16";
        maxBankPreset = 4;
    }
    else
    {
...
{
    return m_isIrEnabled;
}

void Preset::setIsIrEnabled(bool newIsIrEnabled)
{
    m_isIrEnabled = newIsIrEnabled;
}
